...
public class DisplayDetector {
    ...
    /**
     * Detecta dígitos en un Bitmap usando el modelo TFLite.
     * Ejecuta la detección en un hilo separado y retorna los resultados por callback.
    ...
    public void detect(@NonNull Bitmap bitmap, @NonNull DetectionCallback callback) {
        executor.execute(() -> {
            try {
                // Procesar la imagen y obtener las detecciones
    ...
    /** Detección interna (sincrónica).
     * @param bitmap Imagen de entrada
     * @return Lista de detecciones
     */
    private List<DetectionResult> detectInternal(@NonNull Bitmap bitmap) {
        long t0 = System.currentTimeMillis();
        // Preprocesar la imagen para el modelo (resize y normalización)
        ByteBuffer input = preprocess(bitmap);

        // Obtener la forma y tipo de la salida del modelo
        ...
        // Determinar el layout de la salida (canal primero o punto primero)
        ...
        // Parsear las predicciones según el layout
    ...
    private ByteBuffer preprocess(@NonNull Bitmap src) {
        ...
        // Crear el buffer para los datos normalizados (float32)
        ByteBuffer buf = ByteBuffer.allocateDirect(1 * INPUT_SIZE * INPUT_SIZE * 3 * 4);
        buf.order(ByteOrder.nativeOrder());
        buf.rewind();

        // Extraer los píxeles de la imagen redimensionada
    ...
    /**
     * Agrega una detección a la lista, escalando la caja a la imagen original.
    ...
    private void addBox(List<DetectionResult> list, float cx, float cy, float w, float h,
        ...
        // Verificar que la caja tenga tamaño válido
    ...
    /**
     * Aplica supresión de no-máximos (NMS) agnóstico de clase para filtrar detecciones solapadas.
    ...
    /**
     * Calcula el IoU (intersección sobre unión) entre dos cajas.
    ...
    /** Libera los recursos del intérprete y TFLite y apaga el executor (hilo). */
    public void close() {
    ...
    /**
     * Clase que representa el resultado de una detección de dígito.
     * Incluye la etiqueta, confianza y caja delimitadora.
     */
